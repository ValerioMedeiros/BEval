...
public class Control {
	...
	void loadConfig() {
		...
		try {
			...
			currentPath = Control.class.getProtectionDomain().getCodeSource()
					.getLocation().getPath().replaceAll(Installation.filenameJar, "");
	...
	public void setGoalParameters(String[] args) {
		...
		goal = tmpGoal.toString().replaceAll("\\\"([^<]*)\\\"", "btrue"); //Remove description tex on PO
		hypothesis = tmpHypothesis.toString().replaceAll("\\\"([^<]*)\\\"", "btrue"); //Remove description tex on PO
		
		//writeFile(modulePath + "_" + expressionName + ".txt", goal);
	...
	void addRuleInPMMFile(boolean poTypedWD) {
		...
		if(poTypedWD)
			fileNameToAddRule = new String(modulePath.substring(0, modulePath.length() - 4)+ "_wd.pmm");
		else
			fileNameToAddRule = new String(modulePath.substring(0, modulePath.length() - 3)+ "pmm");
		...
		else numberTheories=theoriesSplitted.length-1;
		...
		/*		}
				...
				stringB.append(theoriesSplitted[i]);
			}*/
		}
		...
		String addedRule = new String(separator+ "THEORY RulesProB" +expressionName.replace(".", "_") + " IS \n\n"+
				"\n\t /* Expression from " + expressionName +", it was added  in "+new Date()
	...
	public int callProbLogicEvaluator(boolean poTypedWD, boolean addRule,String parameters, String goalExpression ) {
		...
		try {
			...
			writeFile(modulePath + "_" + expressionName + ".txt", goalExpression);
			
			Process proc = rt.exec(this.getExecutablePath() + " "
					+ parameters.replace("\n", "") + " --eval_rule_file "
					+ modulePath + "_" + expressionName + ".txt");
